...
var nativeFullScreenSupport = (
  ...
  (typeof testVideoElement.requestFullscreen != 'undefined') ? 'w3c':
    false);
try{
  require("nw.gui")
  nativeFullScreenSupport = 'nw'
}catch(e){
  console.log(e);
}
...
FlareVideo.fn.fullScreen = function(state){
  ...
  if (window != top) {
    try{
      _window = parent._global._openingWindows.getCOMById('flash-app-window');
    } catch(e){
      console.log(e);
    }
  };
  ...
  default: {
    this.fullWindow(state);
  }
  }
};

FlareVideo.fn.fullWindow = function(state){
  if (typeof state == "undefined") state = true;
...
FlareVideo.fn.createRange = function(){
  if (nativeRangeSupport) {
    ...
    result.setOptions = function(options_){
      result.attr(options_);
      result.val(options_.value).slider("refresh");
    }
  } else {
    var result = $("<div />");    
    if (!result.slider)
